...
import (
	...
	webappv1 "github.com/example/mybookredis/api/v1"
)
...
func (r *GuestBookReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	ctx = context.WithValue(ctx, "guestbook", req.NamespacedName)
	log := log.FromContext(ctx) // r.Log.WithValues("guestbook", req.NamespacedName)
	...
	book.Status.URL = "this-is-book-url" //urlForService(svc, book.Spec.Frontend.ServingPort)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			...
			handler.EnqueueRequestsFromMapFunc(
				handler.MapFunc(r.booksUsingRedis))).
		//  handler.ToRequestsFunc(r.booksUsingRedis))).
		Complete(r)
}
